...
public class P2_Forsaken_DrippyRelative : SplatoonScript<P2_Forsaken_DrippyRelative.Config>
{
    ...
    {
        En =
            "Forsaken - Drippy Relative is a dynamic P2 Missing helper. Settings are limited to global priority, optional pairs, wave sets, and compact role placement tables.",
        Jp =
            "Forsaken - Drippy Relative はP2ミッシング用の動的補助です。設定は全体優先順位、任意ペア、Wave処理セット、役割ごとの配置表に絞っています。"
    };
    ...
    private static readonly InternationalString UseRelativePositionForSameMarkerText = new()
    {
        En = "Use relative position for same marker",
        Jp = "同マーカー左右を相対位置で決定"
    };
    ...
    private string _lastRelativeSameMarkerDebug = "none";
    private string _lastRelativeSameMarkerLog = "";

    public override HashSet<uint>? ValidTerritories { get; } = [TerritoryDancingMadUltimate];
    public override Metadata Metadata => new(1, "Garume, louxsye", "P2 Forsaken - Drippy Relative");
    ...
    private void DrawDebugSettings()
    {
        ...
        ImGui.TextUnformatted($"Active tower positions: {FormatActiveTowerPositions()}");
        ImGui.TextUnformatted(TryGetActiveTowerCenter(out var activeTowerCenter)
            ? $"Tower center: {FormatVector3(activeTowerCenter)}"
            : "Tower center: none");
        ...
        PatternInfo pattern)
    {
        ...
        var sameDebuffPlayers = sideGroup
            ...
            .ToList();
        var priorityDebuffRank = debuff == LiveDebuffKind.None
            ? 0
            : GetPrioritySameDebuffRank(sameDebuffPlayers, me);
        var rankResult = ResolveSameMarkerDebuffRank(me, side, debuff, sameDebuffPlayers, priorityDebuffRank);
        var debuffRank = rankResult.Rank;
        var rankDetail = rankResult.Detail;
        if (!rankResult.UsedRelative)
        {
            var adjustedRank = AdjustInitialHeadRankFromExplicitPair(me, side, debuff, debuffRank);
            if (adjustedRank != debuffRank)
                rankDetail = $"{rankDetail}; initial head rank adjusted to {adjustedRank}";
            debuffRank = adjustedRank;
        }
        ...
        RememberRelativeSameMarkerDebug(me, rankDetail, debuffRank);
    ...
    private (int Rank, bool UsedRelative, string Detail) ResolveSameMarkerDebuffRank(
        IPlayerCharacter target,
        ParticipantSide side,
        LiveDebuffKind debuff,
        IReadOnlyList<IPlayerCharacter> sameDebuffPlayers,
        int priorityRank)
    {
        var fallbackDetail =
        ...
        if (!C.UseRelativePositionForSameMarker)
            return (priorityRank, false, $"{fallbackDetail}; fallback=relative disabled");

        if (side != ParticipantSide.ResolvingGroup)
            return (priorityRank, false, $"{fallbackDetail}; fallback=not resolving group");

        if (sameDebuffPlayers.Count != 2)
            return (priorityRank, false, $"{fallbackDetail}; fallback=same debuff count {sameDebuffPlayers.Count}");
        ...
        string fallbackDetail)
    {
        ...
        if (!TryGetRelativeLocalX(other.Position, referenceTower, pairedTower, out var otherLocalX, out var otherFailure))
            return (priorityRank, false, $"{fallbackDetail}; fallback=other localX unavailable ({otherFailure})");

        if (!IsFinite(myLocalX) || !IsFinite(otherLocalX))
            return (priorityRank, false,
                $"{fallbackDetail}; fallback=localX not finite my={myLocalX:0.000} other={otherLocalX:0.000}");

        var difference = MathF.Abs(myLocalX - otherLocalX);
        if (difference < 0.01f)
            return (priorityRank, false,
                $"{fallbackDetail}; fallback=localX tie my={myLocalX:0.000} other={otherLocalX:0.000}");

        var relativeRank = myLocalX < otherLocalX ? 1 : 2;
    ...
    {
        for (var i = 0; i < sameDebuffPlayers.Count; i++)
            if (sameDebuffPlayers[i].EntityId == target.EntityId)
                return i + 1;

        return 0;
    }

    private void RememberRelativeSameMarkerDebug(IPlayerCharacter player, string detail, int finalRank)
    {
        var basePlayer = BasePlayer;
        if (basePlayer == null || basePlayer.EntityId != player.EntityId) return;

        _lastRelativeSameMarkerDebug = $"{detail}; finalRank={finalRank}";
        DebugLogOnce(ref _lastRelativeSameMarkerLog, _lastRelativeSameMarkerDebug,
            $"RELATIVE_RANK {_lastRelativeSameMarkerDebug}");
    }
    ...
    private bool TryGetActiveTowerPositions(
        out Vector3 referenceTower,
        out Vector3 pairedTower,
        out uint referenceMapPosition,
        out uint pairedMapPosition)
    {
        referenceTower = Vector3.Zero;
        pairedTower = Vector3.Zero;
        referenceMapPosition = 0;
        pairedMapPosition = 0;

        if (!_hasTowerReference || !IsTowerMapPosition(_referenceMapPosition))
            return false;

        referenceMapPosition = _referenceMapPosition;
        pairedMapPosition = AddMapSteps(_referenceMapPosition, 2);
        referenceTower = TowerPosition(referenceMapPosition);
        pairedTower = TowerPosition(pairedMapPosition);
        return true;
    }

    private bool TryGetActiveTowerCenter(out Vector3 center)
    {
        center = Vector3.Zero;
        if (!TryGetActiveTowerPositions(out var referenceTower, out var pairedTower, out _, out _))
            return false;

        center = (referenceTower + pairedTower) * 0.5f;
        return true;
    }

    private bool TryGetRelativeLocalX(
        Vector3 playerPosition,
        Vector3 referenceTower,
        Vector3 pairedTower,
        out float localX,
        out string failureReason)
    {
        ...
        var towerCenter = (referenceTower + pairedTower) * 0.5f;
        var forward = ArenaCenter - towerCenter;
        forward.Y = 0f;
        if (forward.LengthSquared() < 0.0001f)
        {
            failureReason = "tower center overlaps arena center";
            return false;
        }

        forward = Vector3.Normalize(forward);
        var right = new Vector3(forward.Z, 0f, -forward.X);
        if (C.InvertRelativeSameMarkerSide)
            right = -right;

        var offset = playerPosition - towerCenter;
        offset.Y = 0f;
        localX = Vector3.Dot(offset, right);
        if (IsFinite(localX))
            return true;

        failureReason = $"localX not finite ({localX:0.000})";
        return false;
    }
    ...
    private string FormatActiveTowerPositions()
    {
        if (!TryGetActiveTowerPositions(out var referenceTower, out var pairedTower, out var referenceMapPosition,
                out var pairedMapPosition))
            return "none";

        return $"{FormatMapPosition(referenceMapPosition)}={FormatVector3(referenceTower)}, {FormatMapPosition(pairedMapPosition)}={FormatVector3(pairedTower)}";
    }
    ...
    {
        PluginLog.Information($"[DMU P2 Forsaken - Drippy Relative] {message}");
    }
    ...
    private void ResetState()
    {
        ...
        _lastInstructionLog = "";
        _lastRelativeSameMarkerDebug = "none";
        _lastRelativeSameMarkerLog = "";
        Controller.GetRegisteredElements().Each(x => x.Value.Enabled = false);
    }
    ...
    public sealed class Config 
    {
        ...
        {
            Name = "Forsaken - Drippy Relative global priority",
            Description = "Used for priority pairs 1+3, 2+4, 5+7, 6+8 and same-debuff fallback rank ordering. Default: H2 H1 OT MT M1 M2 R1 R2.",
            PriorityLists =
            [
                new PriorityList
                {
        ...
        private void MigrateUiTerminology()
        {
            ...
            const string newPriorityDescription =
                "Used for priority pairs 1+3, 2+4, 5+7, 6+8 and same-debuff fallback rank ordering. Default: H2 H1 OT MT M1 M2 R1 R2.";
        ...
        private void EnsurePairSettings()
        {
            ...
            for (var i = 0; i < Pairs.Length; i++)
            {
                ...
                Pairs[i].Name = $"Forsaken - Drippy Relative Pair {i + 1}";
                Pairs[i].Description =
    ...
    private static PriorityData2[] CreateEmptyPairSettings()
    {
        return Enumerable.Range(0, PairCount)
            ...
            {
                Name = $"Forsaken - Drippy Relative Pair {index + 1}",
                Description =
                    "Explicit Forsaken pair. Pairs containing one head-stack become the first set; pairs with two fan/circle players become the second set."
            })
            .ToArray();
