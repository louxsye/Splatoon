...
internal class P2_Missing_1238_4567_KTBased_BossDistanceAdjust : SplatoonScript
{
    #region Metadata

    public override Metadata? Metadata => new(1, "mirage, louxsye", "P2 Missing 1238/4567 KT Boss Distance Adjust");
    ...
    private string _lastSameDebuffRankDebug = "none";
    ...
    private void DrawPrioritySettings()
    {
        ...
        ImGui.TextUnformatted("Same tower debuff pair: closer to boss = Priority1, farther from boss = Priority2.");
    ...
    private void DrawDebugStepSection()
    {
        ...
        if(_hasActiveTowers && _step is >= ActiveStepMin and <= ActiveStepMax)
        {
            ...
            ImGui.TextUnformatted($"Same-debuff rank: {_lastSameDebuffRankDebug}");
        }
    }
    ...
    {
        C.EnsureDefaults();
        ImGui.TextUnformatted("Player infos (live, BossDistanceAdjust)");
        ImGui.Separator();
    ...
    private string? ResolvePattern211(PlayerInfo info)
    {
        ...
        if(IsTower(info))
        {
            ...
            {
                DebuffKind.Stack => GetSameDebuffTowerRankByBossDistance(
                    _infos.Where(i => IsTower(i) && i.Debuff == DebuffKind.Stack), info, "211_Stack") switch
                {
    ...
    private string? ResolvePattern022(PlayerInfo info)
    {
        if(IsTower(info))
        {
            ...
            {
                return GetSameDebuffTowerRankByBossDistance(
                    _infos.Where(i => IsTower(i) && i.Debuff == DebuffKind.Spread), info, "022_Spread") switch
                {
            ...
            {
                return GetSameDebuffTowerRankByBossDistance(
                    _infos.Where(i => IsTower(i) && i.Debuff == DebuffKind.Cone), info, "022_Cone") switch
                {
                    0 => "022_ConePriority1",
                    1 => "022_ConePriority2",
                    _ => null,
    ...
    private int GetSameDebuffTowerRankByBossDistance(IEnumerable<PlayerInfo> subset, PlayerInfo target, string label)
    {
        var players = OrderInfosByPriority(subset).ToList();
        var priorityRank = players.FindIndex(i => i.Player.EntityId == target.Player.EntityId);
        var fallbackPrefix = $"{label} priorityRank={priorityRank}";

        if(players.Count != 2)
            return RememberSameDebuffRank(target, priorityRank, $"{fallbackPrefix} fallback=count {players.Count}");

        if(priorityRank < 0)
            return RememberSameDebuffRank(target, priorityRank, $"{fallbackPrefix} fallback=target missing");

        var other = players.FirstOrDefault(i => i.Player.EntityId != target.Player.EntityId);
        if(other == null)
            return RememberSameDebuffRank(target, priorityRank, $"{fallbackPrefix} fallback=partner missing");

        if(!TryGetHorizontalDistance(target.Player.Position, ArenaCenter, out var myDistance))
            return RememberSameDebuffRank(target, priorityRank, $"{fallbackPrefix} fallback=my distance unavailable");

        if(!TryGetHorizontalDistance(other.Player.Position, ArenaCenter, out var otherDistance))
            return RememberSameDebuffRank(target, priorityRank, $"{fallbackPrefix} fallback=partner distance unavailable");

        var difference = MathF.Abs(myDistance - otherDistance);
        if(difference < 0.01f)
            return RememberSameDebuffRank(target, priorityRank,
                $"{fallbackPrefix} fallback=distance tie my={myDistance:0.000} other={otherDistance:0.000}");

        var distanceRank = myDistance < otherDistance ? 0 : 1;
        return RememberSameDebuffRank(target, distanceRank,
            $"{label} bossDistanceRank={distanceRank} priorityRank={priorityRank} my={FormatPlayerDebuff(target)} myDistance={myDistance:0.000} partner={FormatPlayerDebuff(other)} partnerDistance={otherDistance:0.000} farAdjusts=Priority2");
    }

    private int RememberSameDebuffRank(PlayerInfo target, int rank, string detail)
    {
        var basePlayer = BasePlayer;
        if(basePlayer != null && basePlayer.EntityId == target.Player.EntityId)
            _lastSameDebuffRankDebug = $"{detail} finalRank={rank}";
